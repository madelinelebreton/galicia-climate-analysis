...
library(ggplot2)
library(DT)
library(leaflet) # for spatial rendering
...
server <- function(input, output, session) {
  ...
  observeEvent(input$btn_fetch, {
    ...
    # Clear previous cache/selections so unselected data doesn't persist
    rv$climate_long   <- NULL
    rv$climate_wide   <- NULL
    rv$climate_matrix <- NULL
